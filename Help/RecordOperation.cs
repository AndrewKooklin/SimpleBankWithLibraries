    ...
    public class RecordOperation
    {
        ...
        public void RecordOperationToBD(string role, string operation, int? totalSum)
        {
            try
            {
                ...
                string stringQuery =  $"INSERT INTO UserOperations(Role, DataOperation, Operation, TotalSum) VALUES ('{role}'," +
                                      $" '{dateTimeOperation}', '{operation}', '{totalSum}')";

                SqliteCmd.Connection = connection;
                SqliteCmd.CommandText = stringQuery;
                var resultTotalSalary = SqliteCmd.ExecuteNonQuery();
                connection.Close();
            }
            catch (Exception ex)
            {
